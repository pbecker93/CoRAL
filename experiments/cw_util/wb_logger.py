...
class CustomWandBLogger(cw_logging.AbstractLogger):

    def __init__(self, ignore_keys: Optional[Iterable] = None, allow_keys: Optional[Iterable] = None):
        super(CustomWandBLogger, self).__init__(ignore_keys=ignore_keys, allow_keys=allow_keys)
        self.log_path = ""
        self.run = None
    ...
    def init_fields(self,  config: Dict, rep: int, rep_log_path: str):
        ...
        self.job_name = self.job_name.replace("metaworld", "mw")
        self.job_name = self.job_name.replace("True", "T")
        self.job_name = self.job_name.replace("False", "F")
