...
# ============================================================================
# Copied and modified from dmc_control/locomotion/arenas/corridor.py
...
class EmptyCorridor(Corridor):
    ...
    def _build(self,
        ...
        self._mjcf_root.asset.add('texture',
                                  ...
                                  width=100,
                                  height=600)
        self._mjcf_root.visual.headlight.set_attributes(ambient=[0.4, 0.4, 0.4],
                                                        diffuse=[0.8, 0.8, 0.8],
                                                        specular=[0.1, 0.1, 0.1])

        alpha = _DEFAULT_ALPHA if visible_side_planes else 0.0
        self._ground_plane = self._mjcf_root.worldbody.add('geom',
                                                           type='plane',
                                                           rgba=[0.5, 0.5, 0.5, 1],
                                                           size=[1, 1, 1])
        self._left_plane = self._mjcf_root.worldbody.add('geom',
                                                         type='plane',
                                                         xyaxes=[1, 0, 0, 0, 0, 1],
                                                         size=[1, 1, 1],
                                                         rgba=[1, 0, 0, alpha])
        self._right_plane = self._mjcf_root.worldbody.add('geom',
                                                          type='plane',
                                                          xyaxes=[-1, 0, 0, 0, 0, 1],
                                                          size=[1, 1, 1],
                                                          rgba=[1, 0, 0, alpha])
        self._near_plane = self._mjcf_root.worldbody.add('geom',
                                                         type='plane',
                                                         xyaxes=[0, 1, 0, 0, 0, 1],
                                                         size=[1, 1, 1],
                                                         rgba=[1, 0, 0, alpha])
        self._far_plane = self._mjcf_root.worldbody.add('geom',
                                                        type='plane',
                                                        xyaxes=[0, -1, 0, 0, 0, 1],
                                                        size=[1, 1, 1],
                                                        rgba=[1, 0, 0, alpha])
...
class GapsCorridor(EmptyCorridor):
    ...
    def _build(self,
        ...
        if self._aesthetic != 'default':
            ...
            self._ground_texture = self._mjcf_root.asset.add('texture',
                                                             name='aesthetic_texture',
                                                             file=ground_info.file,
                                                             type=ground_info.type)
            self._ground_material = self._mjcf_root.asset.add('material',
    ...
    def regenerate(self, random_state):
        ...
        platform_size = [platform_length / 2,
        ...
        while current_x < self._current_corridor_length:
            ...
            platform_pos = [current_x + platform_length / 2.,
                            0,
                            -_WALL_THICKNESS,
                            ]
            platform_size = [platform_length / 2,
            ...
            current_x += platform_length + variation.evaluate(self._gap_length, random_state=random_state)
...
class WallsCorridor(EmptyCorridor):
    ...
    def _build(self,
        ...
        self._cmap = matplotlib.cm.get_cmap('Greys')
    ...
    def regenerate(self, random_state):
        ...
        while wall_x < self._current_corridor_length:
            ...
            wall_rgba = self._cmap(random_state.rand())
